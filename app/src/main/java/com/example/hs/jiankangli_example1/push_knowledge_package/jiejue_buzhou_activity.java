 ...
 */
public class jiejue_buzhou_activity extends AutoLayoutActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.jiejuebuzou_layout);
        ...
        //获取从描述页传过来的数据
        bzIntent = getIntent();
        bundle = bzIntent.getBundleExtra("miaosu");
        tv = bzIntent.getStringExtra("jieshi");//如果tv为空，那么过来的是描述页，不为空，那么是可能页
        //初始化界面控件实例
        initView();
        //关于GridView
        aboutGridView();
        results = bzIntent.getStringExtra("my_draft");
        if(results !=null&&!results.isEmpty()){//如果是从我的草稿页跳转过来的
            ...
            if(my_draft.getBody()!=null){
                if (my_draft.getBody().getData()!=null){
                    if(tv!=null){
                        et_jiejuebuzou_id.setText(my_draft.getBody().getData().getExplanation());//得到解释
                    }else{
                        et_jiejuebuzou_id.setText(my_draft.getBody().getData().getStepsResolve());//得到解决步骤
                    }
                    if(my_draft.getBody().getData().getContentImages()!=null){
                        ...
                        for(My_draft.BodyBean.DataBean.ContentImagesBean Images: contentImages){
                            if(Images.getType()==2){//如果是解决步骤的图片
                            ...
                            }else if(Images.getType()==5){//如果是解释的图片
                                urlList.add(Images.getImagePath());
                                arrayList.add(Images.getLocalImagePath());//将草稿箱中已经选择好的放入arraylist中
                                priviewList.add(Images.getImagePath());
                            }
        ...
        gv_id.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                if(i==gv_id.getAdapter().getCount()-1&&urlList.get(i).equals("tianjia")){//为最后一项，并且最后一项为可以添加
                    //开始选择图片的操作
                    multiImageSelector = MultiImageSelector.create(jiejue_buzhou_activity.this);
                    ...
                    multiImageSelector.start(jiejue_buzhou_activity.this,2);//设置请求码,图片选择器启动
                }
            }
        });
            ...
            public void run() {
                ...
                if(urlList.size()>0){
                    for(int i=0;i<urlList.size();i++){
                        ...
                        new headPicUtlis(jiejue_buzhou_activity.this,null,null).saveBitmap(b,fileName);
    ...
    private void initView() {
        ...
        tt_content = (TextView) findViewById(R.id.tt_content);
        if(tv!=null){
            tt_content.setText(tv);//如果是tv不为空,那么为解释页，设置标题为解释
        }
        gv_id = (GridView)findViewById(R.id.gv_id);//发现girdView
        //发现下一步按钮
        btn_next_id = (Button) findViewById(R.id.btn_next_id);
        et_jiejuebuzou_id = (EditText) findViewById(R.id.et_jiejuebuzou_id);
        btn_next_id.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //跳转到下一界面
                Intent intent1=new Intent(jiejue_buzhou_activity.this,jieguo.class);
                if(tv!=null){//如果是tv不为空,那么为解释页
                    bundle.putString("explanation",et_jiejuebuzou_id.getText().toString());//放入解释
                    if(results!=null&&!results.isEmpty()){
                        intent1.putExtra("my_draft",results);
                    }
                    intent1.putExtra("zhidao","解决指导");
                }else{
                    bundle.putString("steps_resolve",et_jiejuebuzou_id.getText().toString());//解决步骤
                    if(results!=null&&!results.isEmpty()){
                        intent1.putExtra("my_draft",results);
                    }
                }
                bundle.putStringArrayList("Picjiejuebuzhou",arrayList);//返回的当前页面的图片
                bundle.putStringArrayList("PriviewList2",priviewList);//预览
                intent1.putExtra("jiejue",bundle);
        ...
        btn_save_id.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                    ...
                    try {
                        ...
                        if(tv!=null){//如果为解释页
                            jsonObject.put("possible_causes",bundle.getString("possible_causes"));//可能的原因
                            jsonObject.put("explanation",et_jiejuebuzou_id.getText().toString());//解释
                            jsonObject.put("fault_description","");//错误描述
                            jsonObject.put("steps_resolve","");
                            jsonObject.put("err_code","");
                        }else {
                            jsonObject.put("err_code",bundle.getString("err_code"));
                            jsonObject.put("fault_description",bundle.getString("fault_description"));//错误描述
                            jsonObject.put("steps_resolve",et_jiejuebuzou_id.getText().toString());//解决步骤
                            jsonObject.put("possible_causes","");//设置结果
                            jsonObject.put("explanation","");//设置结果
                        }
                        jsonObject.put("result","");//设置
                        jsonObject.put("title",bundle.getString("title"));//设置标题
                        jsonObject.put("summay_content",bundle.getString("summay_content"));
                        jsonObject.put("is_draft",1);//设置为保存
                        jsonObject.put("solving_guide","");//设置解决指导
                        ...
                        for(int i=0;i<arrayList.size();i++){
                            ...
                            if(tv!=null){
                                jsonObject1.put("type",5);//如果当前页面为解释，解释
                            }else{
                                jsonObject1.put("type",2);//如果当前页面为解决步骤，解决步骤
                            }
                            jsonObject1.put("weight",i);
                            jsonArray.put(jsonObject1);
                        }
                        jsonObject.put("pictures",jsonArray);
                        RequestNet.requestServer(jsonObject,URL,jiejue_buzhou_activity.this,"保存草稿");
                    } catch (JSONException e) {
                        e.printStackTrace();
                    } catch (DbException e) {
                        e.printStackTrace();
