...
public class MyAdapter extends BaseAdapter {
	...
	private ArrayList<String> skillList=new ArrayList<>();//将已经选择的传过来

	// 构造器
	public MyAdapter(ArrayList<String> list, Context context, ArrayList<String> skillList) {
	...
	private void initDate() {
		for (int i = 0; i < list.size(); i++) {//将数据源遍历，然后根据数据源的长度来将所有的变为
			//当出现选择的时候
			if(skillList.contains(list.get(i))){
